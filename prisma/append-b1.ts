...
async function appendB1IfMissing() {
  for (const courseSeed of COURSE_SEEDS) {
    ...
    const hasB1 = await prisma.unit.findFirst({ where: { courseId: course.id, level: "B1" } });
    if (hasB1) {
      continue;
    }

    const built = buildCourseContent(courseSeed);
    const units = built.units.filter((unit) => unit.level === "B1");
